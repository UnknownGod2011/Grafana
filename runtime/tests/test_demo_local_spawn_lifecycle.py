...
def _fake_process(*, poll_result=None):
    process = MagicMock(); process.pid = 424242; process.poll.return_value = poll_result; return process


def test_spawn_api_removes_pid_file_when_child_exits_before_health(monkeypatch, tmp_path):
    pid_path = tmp_path / "stageguard-api.pid"; log_path = tmp_path / "stageguard-api.log"; hmac_path = tmp_path / "checkpoint-hmac-key"; process = _fake_process(poll_result=7)
    monkeypatch.setattr(DEMO, "PID_PATH", pid_path); monkeypatch.setattr(DEMO, "API_LOG", log_path); monkeypatch.setattr(DEMO, "HMAC_KEY_PATH", hmac_path); monkeypatch.setattr(DEMO, "STATE_DIR", tmp_path); monkeypatch.setattr(DEMO, "_api_running", lambda: False); monkeypatch.setattr(DEMO.subprocess, "Popen", lambda *args, **kwargs: process)
...
def test_spawn_api_timeout_terminates_only_the_process_it_spawned(monkeypatch, tmp_path):
    pid_path = tmp_path / "stageguard-api.pid"; log_path = tmp_path / "stageguard-api.log"; hmac_path = tmp_path / "checkpoint-hmac-key"; process = _fake_process(poll_result=None); ticks = iter((0.0, 31.0, 31.0))
    monkeypatch.setattr(DEMO, "PID_PATH", pid_path); monkeypatch.setattr(DEMO, "API_LOG", log_path); monkeypatch.setattr(DEMO, "HMAC_KEY_PATH", hmac_path); monkeypatch.setattr(DEMO, "STATE_DIR", tmp_path); monkeypatch.setattr(DEMO, "_api_running", lambda: False); monkeypatch.setattr(DEMO.subprocess, "Popen", lambda *args, **kwargs: process); monkeypatch.setattr(DEMO.time, "monotonic", lambda: next(ticks))
...
def test_reap_spawn_failure_escalates_to_kill_after_bounded_terminate_wait(monkeypatch, tmp_path):
    pid_path = tmp_path / "stageguard-api.pid"; pid_path.write_text("424242\n", encoding="utf-8"); process = _fake_process(poll_result=None); process.wait.side_effect = [subprocess.TimeoutExpired(cmd="stageguard-api", timeout=5), 0]
...
def test_reap_spawn_failure_fails_closed_if_owned_child_survives_kill(monkeypatch, tmp_path):
    pid_path = tmp_path / "stageguard-api.pid"; pid_path.write_text("424242\n", encoding="utf-8"); process = _fake_process(poll_result=None); process.wait.side_effect = [subprocess.TimeoutExpired(cmd="stageguard-api", timeout=5), subprocess.TimeoutExpired(cmd="stageguard-api", timeout=5)]
...
def test_stop_api_signals_only_verified_stageguard_pid_and_clears_metadata(monkeypatch, tmp_path):
    pid_path = tmp_path / "stageguard-api.pid"; pid_path.write_text("424242\n", encoding="utf-8"); states = iter((True, False, False)); kill = MagicMock()
...
def test_stop_api_retains_metadata_when_verified_process_does_not_stop(monkeypatch, tmp_path):
    pid_path = tmp_path / "stageguard-api.pid"; pid_path.write_text("424242\n", encoding="utf-8"); ticks = iter((0.0, 6.0)); kill = MagicMock()
    ...
    kill.assert_called_once(); assert pid_path.exists()
...
def test_stop_api_terminates_verified_owned_process_even_when_health_is_down(monkeypatch, tmp_path):
    ...
    pid_path = tmp_path / "stageguard-api.pid"; pid_path.write_text("424242\n", encoding="utf-8"); kill = MagicMock()
    monkeypatch.setattr(DEMO, "PID_PATH", pid_path); monkeypatch.setattr(DEMO, "_api_running", lambda: False); monkeypatch.setattr(DEMO, "_pid_matches_stageguard_api", lambda pid: pid == 424242); monkeypatch.setattr(DEMO.os, "kill", kill)
    DEMO._stop_api()
...
def test_pid_identity_rejects_different_bootstrap_path_with_same_basename(monkeypatch):
    monkeypatch.setattr(DEMO, "_pid_command", lambda pid: "python /tmp/attacker/bootstrap.py --identity-mode local --port 9110"); assert not DEMO._pid_matches_stageguard_api(7)


def test_pid_identity_requires_flag_value_pairing_not_substring_presence(monkeypatch):
    expected = str(DEMO.RUNTIME / "bootstrap.py"); monkeypatch.setattr(DEMO, "_pid_command", lambda pid: f"python {expected} --identity-mode remote local --port 1234 9110"); assert not DEMO._pid_matches_stageguard_api(7)
