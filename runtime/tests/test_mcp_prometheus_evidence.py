...
def test_expected_labels_bind_evidence_to_requested_series() -> None:
    payload = {"data": [{"metric": {"production_id": "broadcast-beta"}, "value": [1789990000, "9"]}, {"metric": {"production_id": "broadcast-alpha", "region": "west"}, "value": [1789990000, "0.2"]}]}
...
def test_expected_labels_reject_unrelated_series_even_with_valid_sample() -> None:
    payload = {"data": [{"metric": {"production_id": "broadcast-beta"}, "value": [1789990000, "0.2"]}]}
...
def test_expected_labels_must_match_same_series_that_has_sample() -> None:
    payload = {"data": [{"metric": {"production_id": "broadcast-alpha"}}, {"metric": {"production_id": "broadcast-beta"}, "value": [1789990000, "0.2"]}]}
    assert not contains_prometheus_sample(payload, expected_labels={"production_id": "broadcast-alpha"})
...
def test_rejects_metric_metadata_without_sample() -> None:
    assert not contains_prometheus_sample({"data": [{"metric": {"production_id": "broadcast-alpha"}}]})
...
def test_rejects_nested_series_shaped_lookalike_under_data() -> None:
    assert not contains_prometheus_sample({"data": {"metadata": [{"metric": {}, "value": [1789990000, "0.2"]}]}})


def test_rejects_non_string_metric_labels() -> None:
    assert not contains_prometheus_sample({"data": [{"metric": {"production_id": 7}, "value": [1789990000, "0.2"]}]})
...
def test_hostile_dict_subclass_is_opaque_without_items_or_lookup_hooks() -> None:
    ...
    payload = HostileDict({"data": [{"metric": {}, "value": [1789990000, "1"]}]})
    assert not contains_prometheus_sample(payload)


def test_hostile_list_subclass_is_opaque_without_iteration_hooks() -> None:
    ...
    payload = {"data": HostileList([{"metric": {}, "value": [1789990000, "1"]}])}
    assert not contains_prometheus_sample(payload)


def test_hostile_string_subclass_is_not_parsed_or_coerced() -> None:
...
def test_expected_labels_extension_mapping_fails_closed_without_iteration() -> None:
