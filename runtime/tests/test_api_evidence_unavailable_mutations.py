...
class EvidenceUnavailableMutationApiTests(unittest.TestCase):
    def setUp(self):
        self.model = ShouldNotRunModel()
        self.audit = MemoryAuditLog()
        self.remediation = RecordingRemediation()
    ...
    def test_approval_and_execution_stay_inert_during_evidence_outage(self):
        ...
        self.assertIsNone(incident["approval"])
        self.assertIsNone(incident["outcome"])
        self.assert_sanitized(raw)
        ...
        self.assertEqual("only a diagnosed incident can be approved for remediation", body["detail"])
        ...
        status, body, headers, raw = self.request("POST", "/v1/execute", {})
        self.assertEqual(409, status)
        self.assertEqual("invalid_state", body["error"])
        self.assertEqual("matching explicit approval is required before remediation", body["detail"])
        self.assertEqual("no-store", headers.get("Cache-Control"))
        self.assert_sanitized(raw)
        self.assertEqual(before, self.audit.events)
        self.assertEqual([], self.remediation.calls)
        ...
        self.assertEqual(200, status)
        self.assertEqual("no-store", headers.get("Cache-Control"))
        self.assertEqual("abstain", body["incident"]["report"]["status"])
        self.assertEqual(["causal"], body["incident"]["report"]["unavailable_evidence"])
