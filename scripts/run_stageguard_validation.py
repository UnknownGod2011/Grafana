...
Gate order follows authority: validate the harness/configuration and durable state
first, then operator ingress/concurrency and incident lifecycle, then the remediation
adapter boundary, audit/execution contracts, and finally read-only Grafana MCP access.
The remediation gate is explicit because provider/transport/result contracts can
carry infrastructure-write authority even when their filenames do not contain
"execution".
"""
...
GATES = (
    ...
    Gate("remediation adapter boundary", (
        "test_remediation.py", "test_remediation_receiver.py", "test_remediation_result_boundary.py",
        "test_production_remediation.py", "test_http_remediation_transport.py",
        "test_http_remediation_tls_integration.py", "test_http_reconciliation_audit_sequence.py",
        "test_http_subprocess_*.py", "test_production_reconciliation_bootstrap.py",
    )),
    Gate("timeline disclosure", ("test_timeline*.py", "test_audit_timeline*.py")),
    Gate("public audit", ("test_*audit*.py",)),
    Gate("execution safety", ("test_*execution*.py", "test_local_execution_uncertainty_barrier.py")),
...
def _validation_env(source=None, *, isolated_home=None):
    ...
    if isolated_home is not None:
        sanitized["HOME"] = isolated_home; sanitized["USERPROFILE"] = isolated_home
...
def _run_test_file(path: Path, *, timeout: float, env: dict[str, str]) -> int:
    return subprocess.run(_command(path), cwd=ROOT, check=False, timeout=timeout, env=env, stdin=subprocess.DEVNULL).returncode

def main() -> int:
    parser = argparse.ArgumentParser(description="Run dependency-light StageGuard safety/MCP regression gates.")
    parser.add_argument("--keep-going", action="store_true")
    parser.add_argument("--list", action="store_true")
    parser.add_argument("--file-timeout", type=_positive_timeout, default=DEFAULT_FILE_TIMEOUT_SECONDS, metavar="SECONDS")
