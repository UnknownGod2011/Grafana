...
SENSITIVE_ENV_NAMES=frozenset({"GOOGLE_APPLICATION_CREDENTIALS","GOOGLE_CREDENTIALS","GOOGLE_CLOUD_KEYFILE_JSON","CLOUDSDK_AUTH_ACCESS_TOKEN","CLOUDSDK_AUTH_CREDENTIAL_FILE_OVERRIDE","GCE_METADATA_HOST","GCE_METADATA_IP","HTTP_PROXY","HTTPS_PROXY","ALL_PROXY","NO_PROXY","PYTHONHOME","PYTHONPATH","PYTHONSTARTUP","PYTHONINSPECT","PYTHONBREAKPOINT","SSH_AUTH_SOCK","SSH_AGENT_PID","GIT_ASKPASS","SSH_ASKPASS","GIT_SSH","GIT_SSH_COMMAND","NETRC","DOCKER_CONFIG","KUBECONFIG","PIP_INDEX_URL","PIP_EXTRA_INDEX_URL","PIP_CONFIG_FILE","NPM_CONFIG_USERCONFIG","YARN_RC_FILENAME","CURL_HOME","WGETRC","XDG_CONFIG_HOME","XDG_DATA_HOME","APPDATA","LOCALAPPDATA"})
# Keep validation subprocesses from inheriting credentials for adjacent providers/tools.
# The runner is intended to prove dependency-light behavior, so ambient credentials are a
# test-integrity bug even when StageGuard does not currently consume a given provider.
# External config roots are credential channels too: many CLIs discover auth material from
# XDG_CONFIG_HOME or Windows APPDATA even after HOME/USERPROFILE have been isolated.
SENSITIVE_ENV_PREFIXES=("GRAFANA_","GEMINI_","GOOGLE_API_","STAGEGUARD_REMEDIATION_","AWS_","AZURE_","ANTHROPIC_","OPENAI_","GITHUB_","GH_","HF_","HUGGINGFACE_")
...
def _validation_env(source=None,*,isolated_home=None):
 ...
 if isolated_home is not None:
  sanitized["HOME"]=isolated_home;sanitized["USERPROFILE"]=isolated_home;sanitized["XDG_CONFIG_HOME"]=str(Path(isolated_home)/".config");sanitized["XDG_DATA_HOME"]=str(Path(isolated_home)/".local"/"share");sanitized["APPDATA"]=str(Path(isolated_home)/"AppData"/"Roaming");sanitized["LOCALAPPDATA"]=str(Path(isolated_home)/"AppData"/"Local");sanitized["CLOUDSDK_CONFIG"]=str(Path(isolated_home)/".config"/"gcloud")
 return sanitized
